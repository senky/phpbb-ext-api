...
namespace senky\api\tests\functional;
...
class view_test extends \phpbb_functional_test_case
{
	...
	public function test_view_foo()
	{
		...
		$this->add_lang_ext('senky/api', 'common');
		$this->assertContains($this->lang('API_HELLO', 'foo'), $crawler->filter('h2')->text());
		$this->assertNotContains($this->lang('API_GOODBYE', 'foo'), $crawler->filter('h2')->text());

		$this->assertNotContainsLang('ACP_API', $crawler->filter('h2')->text());
	}
